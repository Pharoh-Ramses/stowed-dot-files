...
# Check if wf-recorder is already running, if so, stop it
...
# Start recording with timestamp filename and audio
dateTime=$(date +%m-%d-%Y-%H:%M:%S)
wf-recorder --bframes max_b_frames --audio -f $HOME/Videos/$dateTime.mp4
